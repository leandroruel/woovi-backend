 ...
 */
export const emailExists = async (email) => Boolean(await UserModel.exists({ email }));
/**
 ...
 */
export const documentExists = async (taxId) => Boolean(await UserModel.exists({ tax_id: taxId }));
/**
 * Create a new user
 * @param body {Object} - user data
 * @returns
 */
export const createUser = async (body) => await UserModel.create({ ...body, tax_id: body.taxId });
/**
 *
 ...
 */
export const updateUser = async (id, input) => await UserModel.findByIdAndUpdate(id, { ...input, tax_id: input.taxId }, {
    new: true,
});
/**
