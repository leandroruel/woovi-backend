import { CONECTAGOV_API_URL, CONECTAGOV_CLIENT_ID, CONECTAGOV_CLIENT_SECRET, CONECTAGOV_OAUTH_URL, } from "@/helpers/document";
    ...
    try {
        const options = {
            ...
            headers: {
                ...
                Authorization: `Basic ${Buffer.from(`${CONECTAGOV_CLIENT_ID}:${CONECTAGOV_CLIENT_SECRET}`).toString("base64")}`,
            },
 ...
 * @param ctx {any} Contexto da requisição
 */
const handleTokenRequest = async (ctx) => {
    try {
...
const isTokenExpired = (accessToken) => {
    ...
    return isBefore(expirationDate, Date.now() + EXPIRATION_WINDOW_IN_SECONDS * 1000);
};
export const validateDocument = async (ctx) => {
