import { DOCUMENT_ALREADY_EXISTS, EMAIL_ALREADY_EXISTS, } from "@/helpers/constants";
import { signToken } from "@/helpers/jwt";
import { encryptPassword } from "@/helpers/password";
import { createAccount, generateAccountNumber, } from "@/services/account.service";
import { createUser, documentExists, emailExists, loginUser, logoutUser, updateUser, } from "@/services/user.service";
import { validateUserCreate, validateUserUpdate, } from "@/validators/user-schema";
...
export const update = async (args) => {
    const { id, user: { password, ...rest }, } = args;
    const { error } = await validateUserUpdate.validateAsync({ user: rest }, {
        abortEarly: false,
    });
    if (error)
        throw new GraphQLError(error);
