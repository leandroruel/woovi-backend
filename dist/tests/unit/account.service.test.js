...
import { INSUFFICIENT_BALANCE, SENDER_RECEIVER_NOT_FOUND, TRANSACTION_ALREADY_EXISTS, } from "@/helpers/constants";
vi.mock("@/models/Account");
vi.mock("@/services/transaction.service");
vi.mock("uuid", async () => {
    const originalUuid = await vi.importActual("uuid");
    return {
        ...originalUuid,
        v4: vi.fn(),
        default: vi.fn(),
...
describe("transferAmount", () => {
    ...
    it("should transfer amount between accounts successfully", async () => {
        ...
        uuidv4.mockReturnValue(idempotencyId);
    ...
    it("should throw an error if transaction already exists", async () => {
        ...
        vi.spyOn(AccountModel, "findOne")
            ...
            .mockResolvedValueOnce(receiverAccount);
        uuidv4.mockReturnValue(idempotencyId);
        createTransaction.mockRejectedValueOnce(error);
        await expect(transferAmount({
            transferAmountPayload: {
                senderId,
                receiverId,
